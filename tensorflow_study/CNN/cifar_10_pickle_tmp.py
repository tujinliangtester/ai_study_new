'''
在调试的过程中，发现，如果输入不同，测试出来的结果完全不一样，初步分析，应该是数据的组织有问题
又遇到准确率非常之低的情况了，为什么呢？真的达到一个局部最小值了？
'''
import numpy as np
...
w_fc = tf.Variable(tf.truncated_normal(shape=(3072, 10), stddev=0.1))
b_fc = tf.Variable(tf.ones(shape=10))
fc = tf.matmul(pool_cnn2, w_fc) + b_fc
'''
relu_fc = tf.nn.relu(fc)
w_fc2 = tf.Variable(tf.truncated_normal(shape=(1000, 10), stddev=0.1))
...
learning_rate, n_epoch = learning_rate_min(0.001, n_epoch)
...
with tf.Session() as sess:
    ...
    for i in range(1000):
        ...
        if (i % 10 == 0):
            print(i, 'acc:')
            print(acc.eval(feed_dict={X: images_train_batch, y: label_train_batch}))
        # print(acc.eval(feed_dict={X: images_train_batch, y: label_train_batch}))
