...
class resNet18(Model):
    def __init__(self, *args, **kwargs):
        ...
        resB = resBlock(filters=128, kernel_size=(3, 3), strides=2, jump_step=4)
        self.resNetBlock.add(resB)

        resB = resBlock(filters=128, kernel_size=(3, 3), strides=1, jump_step=3)
        self.resNetBlock.add(resB)
...
# 断点续训
check_point_path='./check_point/20201108/mnist.ckpt'
...
# 保存参数，方便画出整体图形
with open('history_deep.txt','a') as f:
    f.write('acc:')
    f.write(str(history.history['sparse_categorical_accuracy']))
    f.write('val_acc:')
    f.write(str(history.history['val_sparse_categorical_accuracy']))
    f.write('loss:')
